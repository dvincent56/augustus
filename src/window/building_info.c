...
#include "window/building/tollhouse.h"
...
static void draw_background(void)
{
    ...
    if (context.type == BUILDING_INFO_NONE) {
        ...
        } else if (btype == BUILDING_TOLLHOUSE) {
            window_building_draw_tollhouse(&context);
        ...
        } else if (btype == BUILDING_NATIVE_MONUMENT) {
...
static void handle_input(const mouse *m, const hotkeys *h)
{
    ...
    if (!handled && input_go_back_requested(m, h)) {
        if (previous_context.building_id) {
